...
public abstract class TopicDataQosPolicyHelper
{
	...
	public static org.omg.dds.TopicDataQosPolicy read (final org.omg.CORBA.portable.InputStream in)
	{
		...
		int _lresult_value106 = in.read_long();
		try
		{
			 int x = in.available();
			 if ( x > 0 && _lresult_value106 > x )
				{
					throw new org.omg.CORBA.MARSHAL("Sequence length too large. Only " + x + " available and trying to assign " + _lresult_value106);
				}
		}
		catch (java.io.IOException e)
		{
		}
		result.value = new byte[_lresult_value106];
		in.read_octet_array(result.value,0,_lresult_value106);
		return result;
	}

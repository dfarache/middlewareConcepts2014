...
public abstract class ServiceContextHelper
{
	...
	public static org.omg.IOP.ServiceContext read (final org.omg.CORBA.portable.InputStream in)
	{
		...
		int _lresult_context_data57 = in.read_long();
		try
		{
			 int x = in.available();
			 if ( x > 0 && _lresult_context_data57 > x )
				{
					throw new org.omg.CORBA.MARSHAL("Sequence length too large. Only " + x + " available and trying to assign " + _lresult_context_data57);
				}
		}
		catch (java.io.IOException e)
		{
		}
		result.context_data = new byte[_lresult_context_data57];
		in.read_octet_array(result.context_data,0,_lresult_context_data57);
		return result;
	}

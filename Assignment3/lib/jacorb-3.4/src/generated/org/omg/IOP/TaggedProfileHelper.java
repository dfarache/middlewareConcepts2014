...
public abstract class TaggedProfileHelper
{
	...
	public static org.omg.IOP.TaggedProfile read (final org.omg.CORBA.portable.InputStream in)
	{
		...
		int _lresult_profile_data52 = in.read_long();
		try
		{
			 int x = in.available();
			 if ( x > 0 && _lresult_profile_data52 > x )
				{
					throw new org.omg.CORBA.MARSHAL("Sequence length too large. Only " + x + " available and trying to assign " + _lresult_profile_data52);
				}
		}
		catch (java.io.IOException e)
		{
		}
		result.profile_data = new byte[_lresult_profile_data52];
		in.read_octet_array(result.profile_data,0,_lresult_profile_data52);
		return result;
	}

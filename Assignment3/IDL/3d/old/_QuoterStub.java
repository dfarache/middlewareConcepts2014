...
_os.write_string( tmpResult3 );
					_is = _invoke(_os);
					StockExchange.Quote _result = StockExchange.QuoteHelper.read(_is);
					return _result;
				}
				...
				catch( org.omg.CORBA.portable.ApplicationException _ax )
				{
					...
					try
					{
						...
						{
							throw StockExchange.Invalid_StockHelper.read(_ax.getInputStream());
						}
						else 
						{
		...
		else
		{
			...
			catch (StockExchange.Invalid_Stock ex) 
			{
				if ( _so instanceof org.omg.CORBA.portable.ServantObjectExt) 
